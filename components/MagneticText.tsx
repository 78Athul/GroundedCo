...
export function MagneticText({ text = "CREATIVE", hoverText = "EXPLORE", className = "" }: MagneticTextProps) {
  ...
  return (
    <div
      ...
      className={`relative inline-flex items-center justify-center cursor-none select-none mix-blend-multiply opacity-90 ${className}`}
    >
      {/* Base text layer - original text */}
      <span className="font-display text-forest text-[15vw] leading-[0.8] uppercase tracking-tighter">
        {text}
      </span>

      <div
        ref={circleRef}
        className="absolute top-0 left-0 pointer-events-none rounded-full bg-maroon overflow-hidden"
        style={{
      ...
      >
        ...
        >
          <span className="font-display text-cream text-[15vw] leading-[0.8] uppercase tracking-tighter whitespace-nowrap">
            {hoverText}
          </span>
        </div>
      </div>
